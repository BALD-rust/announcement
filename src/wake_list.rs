//! This module belongs to `whisk`, another async channel library.
//! It was modified to (only) support WakeLists that wake all wakers when requested.
//! It can be found at https://github.com/ardaku/whisk.
//! Licensed under Apache-2.0, Boost 1.0 or MIT.
...
impl WakeList {
    ...
    /// Attempt to wake all wakers.
    pub(crate) fn wake_all(&self) {
        // Start from first pointer of the list
        let mut tmp = self.root.load(SeqCst);

        while !tmp.is_null() {
            let next = unsafe { (*tmp).next.load(Relaxed) };
            let _ = unsafe { (*tmp).data.wake() };
            tmp = next;
        }
    }
